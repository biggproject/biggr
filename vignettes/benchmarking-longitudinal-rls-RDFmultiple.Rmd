...
# Detect holidays and define special periods
covid_affected_period <- detect_disruptive_period(data=df, consumptionColumn="Qe", 
  ...
  minIniDate = as.Date("2020-03-14"), maxIniDate = as.Date("2020-04-14"),
  minEndDate = as.Date("2020-06-21"), maxEndDate = as.Date(max(df$time)))

covid_affected_period <- 
  seq.Date(covid_affected_period$minDate,covid_affected_period$maxDate,by = "days")

covid_lockdown_weekdays <- covid_affected_period[!(format(covid_affected_period,"%u") %in% c("6","7"))]
...
holidays_dates <- detect_holidays_in_tertiary_buildings(
  ...
  timeColumn = "time", 
  tz=tz,
  ignoreDates = covid_lockdown_weekdays
