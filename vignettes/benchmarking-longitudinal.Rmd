...
with(mlflow_start_run(experiment_id = experimentId),{
  mod <- train(
    Qe ~ hour + th + tc + thint + tcint + radh + radc,# + wh + wc + isolh + isolc + light,
    data=df,
    method = RLS(
    ...
    tuneGrid = expand.grid(
      "nhar"=c(5),"tbalh"=c(14),"tbalc"=c(24),"alpha"=c(0.2),
      "lambda"=c(
        get_lpf_smoothing_time_scale(data.frame("time"=df$time),0.5*24*365)
      )),
  ...
  df_for_pred <- df[df$time>=as.POSIXct("2018-01-01 00:00:00") & df$time<=as.POSIXct("2018-12-31 23:59:59"),]
...
```{r}
pred_distribution <- 
  predictor_loaded(df_for_pred, model_horizon_in_hours=18*30*24, model_window="%Y-%U", model_selection="rmse") %>%
  left_join(df_for_pred[,c("time","Qe")])
ggplotly(ggplot(pred_distribution) + geom_line(aes(time,Qe)) +
  geom_line(aes(time,mean),col="red", alpha = 0.5) +
  geom_line(aes(time,`5%`),col="blue", alpha=0.3) + geom_line(aes(time,`95%`),col="blue",alpha=0.3))
```
