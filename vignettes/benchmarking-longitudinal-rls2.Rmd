...
```{r}
data(biggr)
buildingId <- "electricity1"
df <- electricity1 %>% calendar_components(localTimeZone = "Europe/Madrid")
...
with(mlflow_start_run(experiment_id = experimentId),{
  mod <- train(
    ...
    method = RLS(
     data.frame(parameter = c('nhar','tbalh','tbalc','alpha','lambda','minocc'),
                class = c('integer','float','float','float','float','float'))
    ),
    tuneGrid = expand.grid(
      # "nhar"=c(4),"tbalh"=c(16),"tbalc"=c(16),"alpha"=c(0.95),
      "nhar"=c(16),"tbalh"=c(16,18,20),"tbalc"=c(22,24,26),"alpha"=c(0.95),
      "lambda"=c(
    ...
    trControl = #trainControl(method="none"),
      trainControl(method = "timeslice",
                   ...
                   skip = hourly_timesteps(365/6*24,detect_time_step(df$time))-1,
                   fixedWindow = T,
    ...
    transformationSentences = list(
     ...
     "weekhour" = c(
       "fs_components(...,featuresName='weekhour',nHarmonics=param$nhar,inplace=F)"
       #"isWeekend"
     ),
...
ggplotly(ggplot(pred_distribution) + geom_line(aes(localtime,Qe)) +
  ...
  geom_ribbon(aes(localtime,ymin=`5.0%`,ymax=`95.0%`),color="blue", alpha=0.3))
```
...
pred_distribution_d <- cbind(
  setNames(
    as.data.frame(mapply(function(v){
      ...
      #rollmean(pred_distribution_d[,v],k = 7,fill = c(NA,NA,NA),align="center",partial=T)
    }, vars_to_resample)),
  paste0(vars_to_resample,"_smoothed")
  ),
  pred_distribution_d)
ggplotly(ggplot(pred_distribution_d) + #geom_line(aes(date,(`mean`-pred)/pred)) +
           geom_hline(aes(yintercept=0), linetype="dashed") +
           geom_ribbon( aes(date, ymin=(`75.0%_smoothed`-`Qe_smoothed`)/`75.0%_smoothed`,
                            ymax=(`25.0%_smoothed`-`Qe_smoothed`)/`25.0%_smoothed`), 
                        fill="grey", alpha=0.5 ) +
           geom_ribbon( aes(date, ymin=(`97.5%_smoothed`-`Qe_smoothed`)/`97.5%_smoothed`,
                            ymax=(`2.5%_smoothed`-`Qe_smoothed`)/`2.5%_smoothed`), 
                        fill="grey", alpha=0.2 ) +
           geom_ribbon( aes(date, ymin=(`90.0%_smoothed`-`Qe_smoothed`)/`90.0%_smoothed`,
                            ymax=(`10.0%_smoothed`-`Qe_smoothed`)/`10.0%_smoothed`), 
                        fill="grey", alpha=0.3 ) +
           geom_line(aes(date,(`mean_smoothed`-`Qe_smoothed`)/`mean_smoothed`), 
                     col="blue", alpha=0.8, size = 1) +
