...
readdata <- function(filename)
{
    ...
    data <- read_delim(filename, ';', col_names = columns,
        col_types=cols(
            time=col_character(),
            value=col_double()))
...
test_that("Fill ts na. No missing hour", {
    ...
    maxGap <- "5H"
    obtained <- fill_ts_na(data, outliersMinMax, outliersZScore,
...
fill_test <- function(data, methodFillNA, maxGap, expected)
{
    ...
    obtained <- fill_ts_na(data, outliersMinMax, outliersZScore,
        outliersCalendarModel, methodFillNA, maxGap)
    expect(all(obtained == expected, na.rm=TRUE),
        "Expected and obtained are different")
}
